...
const AccountEdit = ({ isOpen, onToggle }) => {
    ...
    useEffect(() => {
        const fetchPaises = async () => {
            const response = await axios.get("http://34.171.165.241:4000/api/countries");
            setCountries(response.data.data);
        }
        const fetchTiposDocumentos = async () => {
            const response = await axios.get(
                "http://34.171.165.241:4000/api/tipo_doc"
            );
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.put("http://34.171.165.241:4000/api/users/update", formData);
