...
const AccountProfile = ({ onToggle }) => {
    ...
    const handleDelete = async (e) => {
        ...
        if (confirmacion) {
            try {
                const { request } = await axios.put("http://34.171.165.241:4000/api/users/delete", {
                    correo: auth.correo,
                    estado: "I",
                });
    ...
    const fetchPlan = async () =>{
        const response = await axios.get(
            `http://34.171.165.241:4000/api/plan/currentPlan/${userInfo[0].id_usuario}`
        );
    ...
    const fetchUsers = async () =>{
        const response = await axios.get(
            `http://34.171.165.241:4000/api/domain/${auth["id_usuario"]}`
        );
    ...
    const fetchDomains= async  ()=>{
        const response = await  axios.get(
            `http://34.171.165.241:4000/api/domain/countDomains/${auth["id_usuario"]}`
        );
