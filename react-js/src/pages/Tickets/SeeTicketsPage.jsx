...
const SeeTicketsPage = ({ isValid = true }) => {
    ...
    useEffect(() => {
        const fetchInfo = async () => {
            try {
                const response = await axios.get(
                    `http://34.171.165.241:4000/api/ticket/getInfoTicketEmployee/${ticketStorage}`
                );
        ...
        const fetchResponses = async ()=>{
            try{
                const response = await axios.get(
                    `http://34.171.165.241:4000/api/ticket/getResponses/${ticketStorage}`
                );
    ...
    const handleSubmitResponse = async (event) => {
        ...
        setIsOpen(false)
        try{
            const { data } = await axios.post("http://34.171.165.241:4000/api/ticket/responseTicket", {
                ticketStorage,
    ...
    return (
        <WrappedWidth size={10}>
            ...
            <section className="w-full mt-2 flex flex-col gap-y-4">
                {responseTicketBack && (
                    <TicketResponse
                        state="Resuelto"
