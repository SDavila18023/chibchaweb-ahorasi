...
const AllocationTicketspage = () => {
    ...
    useEffect(() => {
        const fetchInfo = async () => {
            ...
            try {
                const response = await axios.get(
                    `http://34.171.165.241:4000/api/ticket/getInfoTicket/${ticketStorage}`
                );
        ...
        const fetchEmployees = async () => {
            ...
            try {
                const response = await axios.get(
                    "http://34.171.165.241:4000/api/ticket/getEmployees"
                );
                if (response.data.data && response.data.data.length > 0) {
    ...
    const handleSubmit = async (event) => {
        ...
        try{
            const { data } = await axios.post("http://34.171.165.241:4000/api/ticket/assignEmployee", {
                ticketStorage,
                formData
            });
